...
ユーザーが Airbnb の物件 URL を貼り付けると、システムが公開情報を取得し、**5 維度健康診断 + 综合评分 + AI 改善レポート + SOZONEXT サポート CTA**を出力する。

- **ユーザー**：SOZONEXT 内部運営(demo 阶段、主要观众是老板)
- **言語**：UI / PDF / AI レポートすべて **日本語**(コードコメントと本ドキュメントは中文/英文 OK)
- **コスト制約**：月 **$0**(强约束)
...
| [docs/adr/](docs/adr/) | 6 つのアーキテクチャ決定(Vercel+Mac / fetch / Claude SDK / 同期 / 単一 tool_use / 通知メール撤廃) |
...
**月コスト合計 $0**:Vercel Hobby + Neon 無料 + Claude Code サブスクリプション。
...
├── components/              # ScoreCard / QualityStatusLadder / DimensionCard / SupportCta / ...
├── lib/                     # db / scraper(client) / pdf / i18n / util
...
POST /api/diagnose { url }       ← Vercel 60s 予算
  ...
  → 6. { diagnosis_id, redirect } を返す
       (F1 アラートメールは廃止、結果ページの SupportCta が CTA を担う)
```
...
3. Top 3 改善案(`top3`) — 結果ページの Top3Priorities セクションで使用
