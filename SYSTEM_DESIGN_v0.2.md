...
> - **UI / PDF / 邮件 / AI 报告全部日语**（PRD v0.4 对齐）
...
实现 demo 阶段 Airbnb 房源健康诊断系统。用户贴 URL → 抓数据 → 输出 5 维度评分 + AI 报告 + 预警邮件。
...
- 邮件：**Resend** (免费 3000/月，demo 数十封)
- PDF：**@react-pdf/renderer** + **NotoSansJP** 内嵌字体
- 图表：**Recharts**
- 国际化：**next-intl**（即便只有日语，规范化文案）

**总月成本**：$0（Vercel Hobby + Neon 免费 + Resend 免费 + Claude 走订阅）
...
└────────────────────────────────────┼───────┼──────┼────────────┘
            ...
            ▼                                ▼                ▼
┌──────────────────────────┐    ┌──────────────┐    ┌──────────────┐
│ Mac (常駐)                │    │ Resend       │    │ Neon Postgres │
│ Cloudflare Tunnel        │    │ Email API    │    │ (Drizzle ORM) │
│ ┌──────────────────────┐ │    │ - F1 alert   │    │ - listings   │
│ │ Node Fetch Scraper   │ │    │ - F7 manual  │    │ - diagnoses  │
│ │  POST /diagnose      │ │    └──────────────┘    │ - alerts_sent│
│ │  - fetch airbnb HTML │ │                        └──────────────┘
│ │  - parse deferred    │ │
│ │    state JSON        │ │
│ │  - fetch reviews via │ │
...
│       └── weekly/test/route.ts    # POST「立即测试发送」
├── components/
...
│   ├── AlertBanner.tsx             # F1
│   └── WeeklyDigestPanel.tsx       # F7 mock + 测试按钮
├── lib/
│   ├── db/
...
│   ├── email/
│   │   ├── resend.ts
│   │   ├── alert.tsx               # F1 react-email 模板（日语）
│   │   └── weekly.tsx              # F7 模板（日语）
│   ├── pdf/
...
CREATE TABLE alerts_sent (
  diagnosis_id  UUID PRIMARY KEY REFERENCES diagnoses(id),
...
**和 v0.1 差别**：拆出 `ai_top3` 字段（之前藏在 ai_report_md 内），方便 F1 邮件直接读用，不用二次解析 markdown。
...
// 504 { "error": "timeout" }
```
内部：调 Mac scraper（≤45s timeout）→ 拿到完整 diagnosis JSON → 写 DB → 触发 F1 邮件（如需）→ 返回。
...
POST /api/diagnose { url }              ← Vercel ≤60s 预算
  ...
  │       (失败不阻断)
  │
  └─ 7. return { diagnosis_id, redirect }
```
...
| | F1 アラート | F7 週次サマリー |
|---|---|---|
| 触发 | 诊断流程内同步 | `POST /api/weekly/test`（无定时器） |
| 模板 | `lib/email/alert.tsx` (react-email) | `lib/email/weekly.tsx` |
| 主题 | `⚠️ 物件アラート · {title} · 評価 {score}` | `SOZONEXT 物件ヘルス週次サマリー · {date}` |
| 收件人 | env `ALERT_EMAIL_TO` | 同 |
| 语言 | 日本語 | 日本語 |
...
RESEND_API_KEY=re_...
ALERT_EMAIL_TO=test-alerts@sozonext.com
...
| F7 无定时器 | 砍工程 | v1 上 Vercel Cron 或 GitHub Actions |
...
| 天 | 里程碑 |
|---|---|
| D1 | 仓库脚手架 + Neon migration + Resend / Drizzle 装好 + `lib/i18n/ja.ts` |
...
| D5 | 结果页 UI（评分卡 + 5 维度 + 趋势图 mock + 报告面板，全日语） |
| D6 | F1 / F7 日语邮件模板 + PDF 日语布局 + NotoSansJP 嵌入 |
...
7. 低分房源 → 收件箱有日语预警邮件
8. 点击「テスト送信」→ 收件箱有日语周报
9. Neon console 检查 diagnoses 表新行
